...
describe('OpenCloud v1 Universes API', () => {
  ...
  test('Get Universe Place Configuration', () => {
    fetchApi(OpenCloud.v1.universes.postUniverseIdPlacesPlaceIdVersions, {
...
describe('OpenCloud v1 Messaging API', () => {
  test('Publish Message', () => {
    fetchApi(OpenCloud.v1.messaging.postUniversesUniverseIdTopicsTopic, {
...
describe('OpenCloud v1 DataStores API', () => {
  test('Get DataStore Entries', () => {
    fetchApi(OpenCloud.v1.datastores.getUniversesUniverseIdDatastoresDatastoreEntries, {
      universeId: 1534453623,
    }).catch((error: Error) => {
      // Expect unauthorized error since we don't have API key
      expect(error.message).toBe('Invalid response data');
  ...
  test('List DataStores', () => {
    fetchApi(OpenCloud.v1.datastores.getUniversesUniverseIdDatastores, {
      universeId: 1534453623,
      limit: 10,
    }).catch((error: Error) => {
      // Expect unauthorized error since we don't have API key
...
describe('OpenCloud v1 Ordered DataStores API', () => {
  test('Get Ordered DataStore Entry', () => {
    fetchApi(OpenCloud.v1.datastoresOrdered.getUniversesUniverseIdOrderedDataStoresOrderedDataStoreScopesScopeEntries, {
      universeId: '1534453623',
...
describe('OpenCloud v1 Assets API', () => {
  test('Get Assets', () => {
    fetchApi(OpenCloud.v1.assets.getAssetsAssetId, {
      assetId: '123456',
    }).catch((error: Error) => {
      // Expect unauthorized error since we don't have API key
      expect(error.message).toBe('Invalid response data');
