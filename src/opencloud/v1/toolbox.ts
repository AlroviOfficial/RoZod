...
const QueryFacets = z.object({ appliedFacets: z.array(z.string()).nullable(), availableFacets: z.array(z.string()).nullable() });
const VotingModel = z.object({ showVotes: z.boolean(), upVotes: z.number().int(), downVotes: z.number().int(), canVote: z.boolean(), userVote: z.boolean().nullable(), hasVoted: z.boolean(), voteCount: z.number().int(), upVotePercent: z.number().int() });
const CreatorModelV2 = z.object({ creator: z.string().nullable(), userId: z.number().int().nullable(), groupId: z.number().int().nullable(), name: z.string().nullable(), verified: z.boolean().nullable() });
...
const LinkType = z.union([z.literal(0), z.literal(1), z.literal(2), z.literal(3), z.literal(4), z.literal(5), z.literal(6), z.literal(7), z.literal(8), z.literal(9), z.literal(10)]);
const SocialLinkModel = z.object({ linkType: LinkType, url: z.string().nullable(), title: z.string().nullable() });
const PreviewAssetsModel = z.object({ imagePreviewAssets: z.array(z.number()).nullable(), videoPreviewAssets: z.array(z.number()).nullable() });
const Asset = z.object({ id: z.number().int(), name: z.string().nullable(), description: z.string().nullable(), assetTypeId: z.number().int().nullable(), socialLinks: z.array(SocialLinkModel).nullable(), previewAssets: PreviewAssetsModel.nullable(), createTime: z.string().datetime({ offset: true }).nullable(), updateTime: z.string().datetime({ offset: true }).nullable(), categoryPath: z.string().nullable() });
const Audio = Asset;
const Decal = Asset;
...
const CreatorStoreAsset = z.object({ voting: VotingModel.nullable(), creator: CreatorModelV2.nullable(), creatorStoreProduct: CreatorStoreProduct.nullable(), asset: z.union([Asset, Audio, Decal, MeshPart, Model, Music, Plugin, SoundEffect]).nullable() });
const SearchCreatorStoreAssetsResponse = z.object({ nextPageToken: z.string().nullable(), queryFacets: QueryFacets.nullable(), creatorStoreAssets: z.array(CreatorStoreAsset).nullable(), totalResults: z.number().int(), filteredKeyword: z.string().nullable() });
...
export const getToolboxServiceV2AssetsSearch = endpoint({
	...
	parameters: {
		...
		"creator": z.string().regex(/^(user|group)\/\d+$/).optional(),
